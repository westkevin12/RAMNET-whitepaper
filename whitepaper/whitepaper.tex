...
\begin{enumerate}
    ...
    \item \textbf{Sharding:} Apache Spark orchestrates the splitting of logic into 40/40/40 overlapping segments across 6-node groups.
...
\begin{tikzpicture}[node distance=1.5cm, every node/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, font=\small}]
    % Primary Set
    \node (n1) [fill=blue!10] {Node 1 ($S_1$)};
    \node (n2) [right of=n1, xshift=1.5cm, fill=blue!10] {Node 2 ($S_2$)};
    \node (n3) [right of=n2, xshift=1.5cm, fill=blue!10] {Node 3 ($S_3$)};
    
    % Mirror Set
    \node (n4) [below of=n1, yshift=-0.5cm, fill=green!10] {Node 4 ($S'_1$)};
    \node (n5) [below of=n2, yshift=-0.5cm, fill=green!10] {Node 5 ($S'_2$)};
    \node (n6) [below of=n3, yshift=-0.5cm, fill=green!10] {Node 6 ($S'_3$)};
    
    % Overlap Connectors (Audit Zones)
    \draw[<->, dashed, thick, blue!60] (n1.east) -- node[above, draw=none, fill=none] {10\% Audit} (n2.west);
    \draw[<->, dashed, thick, blue!60] (n2.east) -- node[above, draw=none, fill=none] {10\% Audit} (n3.west);
...
\caption{6-Node ECC Model: 40/40/40 Shard Decomposition with 10\% Overlap Audit Zones and High-Durability Mirroring.}
\label{fig:ecc_model}
\end{figure}

\begin{itemize}
    \item \textbf{40/40/40 Shard Decomposition:} Tasks are partitioned into overlapping segments. This ensures a 10\% overlap between adjacent nodes, allowing for lightweight cross-verification without full task re-execution.
    \item \textbf{Mirror Set (Nodes 4-6):} A simultaneous, ephemeral duplicate of the 40/40/40 split. This set acts as a fallback for surviving regional outages and global network partitions.
\end{itemize}
...
\subsection{The 40/40/40 Decomposition}
Let a compute task $T$ be represented as a set of operations $O$. In a standard RAMNET execution, the task is partitioned into three overlapping segments:
\begin{equation}
S_1 = \{O_1, \dots, O_{0.4n}\}
\end{equation}
\begin{equation}
S_2 = \{O_{0.3n}, \dots, O_{0.7n}\}
\end{equation}
\begin{equation}
S_3 = \{O_{0.6n}, \dots, O_n\}
\end{equation}
This ensures a 10\% overlap between adjacent nodes. These ``Audit Zones'' allow Node A to verify the end-state of Node B’s entry-point without re-executing the entire shard.
